...
public function add() {
	...
	if ($saved) {
		$key = 'your-file-name';
		if (StorageManager::getAdapter('Local')->write($key, file_get_contents($this->data['pdf_files']['file']['tmp_name']))) {
...
```php
StorageManager::getAdapter($data['PdfFile']['adapter'])->delete($data['PdfFile']['path']);
```
